...
Figure \ref{fig:Filtering} shows plots illustrating the filtering process for each Euler angle in a yaw-pitch-roll sequence for a set of 500 nighttime images taken over 50 seconds. Time-domain signals are on the leftmost subplots, and frequency-domain power spectrum plots are on the rightmost subplots, with original signals on top and filtered signals on the bottom.
